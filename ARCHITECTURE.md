...
*(Done 2026-07-13 — **Tasks-grouped-by-category, blank categories hidden** across the three category views (captain call). `js/north.js` "Year vision" now lists only categories that have a goal, each surfaced through its weekly steps (empty → a pointer to Setup, not a blank grid). `js/year-view.js` (Compass) and `js/session-view.js` (Session) do the same but **keep their interactive cards** — goal edit, check-off, and the Session "Mark complete / Share with parents" celebration flow are preserved — with the weekly steps listed inside each card. Planning happens in Setup's guided walkthrough; these pages show its output. **Consequence:** creating a goal for a not-yet-started area (year or later-session) now routes through Setup, since the blank creation-cards are hidden. Composes with the wheel-slice view: a guide-summer life-area with no goal now correctly shows the coverage-frame "quiet" line. SW cache bump on deploy — `north.js`, `year-view.js`, `session-view.js`, `css/style.css` changed.)*
