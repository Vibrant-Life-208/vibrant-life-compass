...
-- Date: 2026-06-29
--
-- The four-login model (captain 2026-06-29): a GUIDE sees their own tribe(s) (the
-- studios they run) + whole-school insights; the OWNER (Jenna) sees whole school +
-- every tribe + her family. A guide can run MORE THAN ONE tribe (e.g. one guide
-- covers Tots + Sparks + Launch Pad), so tribes is an array. Two attributes:
--   tribes   - the studios a guide runs (subset of sparks|discovery|adventure|launchpad|tot)
...
-- would self-grant whole-school visibility. So is_owner AND tribes join role/email/
...
begin
  if coalesce(auth.role(), '') <> 'service_role' and (
    ...
    raise exception 'profiles.role/email/id/is_owner/tribes are not user-writable (TCC v0.14)';
  end if;
  return new;
end;
$$;

-- Verify after applying (anon probe should 200; columns exist):
--   curl -s "$SUPABASE_URL/rest/v1/profiles?select=tribes,is_owner&limit=1" \
